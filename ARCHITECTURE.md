...
| Optional DOCX parsing sources | `pearls/{year}/{originalName}.docx` | cleaner text source for the same-name PDF |
| PDF processing map | `data/pdf-processing-map.json` | forced columns, `sourceOverride`, `showOriginal`, expected document count |
...
For problem PDFs, an editor may place a same-name `.docx` next to the PDF. The parser still treats the PDF as the public/original source, but reads text from the DOCX automatically and records that decision in `processing.sourceOverride`.

`data/pdf-processing-map.json` stores editor-reviewed parsing overrides before JSON generation. Use it for deterministic PDF layout decisions (`columns`), expected internal document counts, optional explicit `sourceOverride` inputs, and `showOriginal` when HTML is not faithful enough for tables or complex layouts. This file may be edited by hand; `data/parsed/` should still be regenerated by the parser/AI pipeline.
