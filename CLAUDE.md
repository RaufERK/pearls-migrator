...
Catalog cards should lead with a clear `Читать` action, show `PDF` as a visible secondary action, and move `DOCX`, `EPUB`, and `TXT` into a compact `Скачать` menu. Do not show `Печать` in the catalog. The material page may list all available download formats, with PDF first, but printing is removed from the MVP UI. Reading on the site remains the primary mobile experience because source PDFs are often two-column and inconvenient on small screens.
