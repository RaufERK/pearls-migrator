...
- [x] Поднять новый проект на сервере через `ssh app` на порту `3021`.
- [x] Через `ssh sw` настроить Nginx/SSL для временного домена `amasters.tech`.
- [x] Проверить `amasters.tech` через `curl`/PM2 logs.
