...
Каталог и страница материала рендерятся на сервере (`force-dynamic`): роботам нужен полный HTML, а не клиентская подгрузка. Редкие (раз в квартал) обновления это окупают. Порядок годов — `src/catalogOrder.ts` (новые сверху). Фильтр года не пишется в URL и сбрасывается при перезагрузке.
