...
import { SiteYearSelect } from './SiteYearSelect';
import type { CatalogFilterLink, CatalogYearGroup, CatalogYearOption, PearlCatalogItem } from '../lib/pearls';
...
export function HomeCatalog({ documentGroups, error, filters, yearLinks }: HomeCatalogProps) {
  const [selectedSiteYear, setSelectedSiteYear] = useState<number | null>(null);
  const visibleGroups = selectedSiteYear
    ? documentGroups.filter((group) => group.year === String(selectedSiteYear))
    : documentGroups;

  useEffect(() => {
    const url = new URL(window.location.href);

    if (!url.searchParams.has('siteYear')) {
      return;
    }

    url.searchParams.delete('siteYear');
  ...
  return (
    <>
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <SiteYearSelect onYearChange={setSelectedSiteYear} selectedSiteYear={selectedSiteYear} yearLinks={yearLinks} />
        {filters.hasActive ? (
      ...
      {error ? (
        <div className="mb-8 rounded-2xl border-2 border-pink-400/50 bg-pink-950/50 p-6 text-pink-100 shadow-xl shadow-pink-500/20">
          ...
          <p className="mt-3 text-sm text-pink-200/80">{error}</p>
        </div>
      ) : null}
...
function DownloadLinks({ item }: { item: PearlCatalogItem }) {
  return (
    <div className="mx-auto grid w-fit grid-cols-2 gap-1.5">
      <a className="flex h-8 w-20 items-center justify-center gap-1 rounded bg-[#0d9e6e]/60 px-3 py-1.5 text-xs font-semibold text-white transition-opacity hover:opacity-90" href={item.path}>
        <BookOpenIcon />
        Читать
      </a>
...
function MobileDownloadLinks({ item }: { item: PearlCatalogItem }) {
  return (
    <div className="pointer-events-auto relative z-10 grid grid-cols-2 gap-2">
      <a className="flex items-center justify-center gap-1 rounded bg-[#0d9e6e]/60 py-1.5 text-xs font-semibold text-white transition-opacity hover:opacity-90" href={item.path}>
        <BookOpenIcon />
        Читать
      </a>
